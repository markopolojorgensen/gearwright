...
func _process(delta):
	...
	if Input.is_action_just_pressed("mouse_rightclick") && hovering:
		if item_popup.visible:
			...
			item_popup.position = global_position + Vector2(200, 0)
			item_popup.popup()

func load_item(a_itemID : String):
	item_data = DataHandler.fish_item_data[a_itemID]
	
	var image = Image.load_from_file(item_data["icon_path"])
	var texture = ImageTexture.create_from_image(image)
	icon.texture = texture
	
	for grid in DataHandler.fish_item_grid_data[a_itemID]:
		var converter_array := []
		for i in grid:
	...
	x_offset = ((41 * icon.scale.x)/2) - 2 + (-30 * lowest_x)
	y_offset = ((39 * icon.scale.y)/2) - 2 + (-31 * lowest_y)
	
	number_bubble.global_position = global_position + Vector2(x_offset, y_offset)
...
func hide_number_label():
	pass
	#number_bubble.visible = false
