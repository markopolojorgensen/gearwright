...
func _ready():
	...
	emit_signal.call_deferred("load_template", template_data[template_data.keys()[0]])

func _on_item_selected(index):
	emit_signal("load_template", template_data[template_data.keys()[index]])
...
func _on_fish_builder_new_save_loaded(a_User_data):
	...
	if a_User_data.has("template") and a_User_data["template"]:
		emit_signal("load_template", template_data[a_User_data["template"]])
